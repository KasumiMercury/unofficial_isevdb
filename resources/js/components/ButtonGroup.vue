            ...
            <div class="py-2 h-100" :style="'background-color:'+current_member.MainCol+';'">
                <div class="d-grid">
                    ...
                    <template v-for="(member,index) in members">
                        <b-button block squared :key="'BtnGroup'+index" class="mt-1 px-4 text-start" :style="'background-color:'+member.BtnCol+';'" v-on:click="trans(member.name)"><i class="fas fa-database"></i>　{{member.display}}DB</b-button>
                    </template>
                        <b-button block squared class="mt-1 px-4 text-start" @click="top"><i class="fas fa-home"></i>　非公式いせぶいDBトップ</b-button>
                        <b-button block size="lg" squared class="mt-3 px-4 text-start" v-on:click="$router.push('/'+current_member.name+'/add')"><i class="fas fa-folder-plus"></i>　非公式{{current_member.display}}DBに<br>データを追加</b-button>
                </div>
            </div>
        </b-sidebar>
    </div>
