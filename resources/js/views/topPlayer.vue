<template>
    <div id="app">
        <b-card title="Card Title" class="my-3" no-body>
            ...
            <b-card-body class="text-center p-3">
                ...
                <div class="d-grid m-3">
                    ...
                    >
                        <b-button
                            class="p-3 mt-3 btn-Tweet text-decoration-none"
                ...
                <div class="d-grid m-3">
                    <b-button
                        ...
                        ><i class="fas fa-home"></i>　非公式いせぶいDBトップへ
                    </b-button>
                    <b-button
                        size="lg"
                        class="my-2 p-3"
                        v-on:click="memberDB(current_member.name)"
                        ><i class="fas fa-database"></i>　このメンバーのDB
                    </b-button>
                </div>
            </b-card-body>
        </b-card>
        <EditModal
            ...
            :player="playerInfo"
            v-on:from-child="hide"
            v-on:from-child2="edited"
        />
        <modal name="completed" class="text-center">
            ...
            <b-button class="m-3" v-on:click="memberDB(current_member.name)"
                >このメンバーのDB</b-button
            >
            <b-button class="m-3" v-on:click="$router.push('/')"
                >非公式いせぶいDBトップ</b-button
            >
        ...
        axios
            ...
            .then(function (response) {
                ...
                if (self.playerInfo.player.end == 0) {
                    self.tag =
                        self.iframe_F +
                        self.playerInfo.player.VideoID +
                        self.iframe_S +
                        self.playerInfo.player.start +
                        self.iframe_L;
                } else {
                    self.tag =
                        self.iframe_F +
                ...
                self.Tweet["title"] =
                    "非公式" +
                    self.playerInfo.player.member.display +
                    "DB No." +
                    self.playerInfo.player.member.id +
                    ":https://isevdb.sakura.ne.jp/" +
                    self.playerInfo.player.member.name +
                    "/player/" +
                    self.id;
                self.Tweet["hash"] =
                    self.playerInfo.player.member.display +
